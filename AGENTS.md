...
- 涉及宿主 hooks、scope 或共享写入：读 [Harness 说明](docs/engineering/agent-harness.md)。
- 涉及实现交付、PR/CR 审查与修复、通用 skill 适配或跨会话接手：读 [交付约定](docs/engineering/development-workflow.md)。
- 处理 issue/spec/review 来源：使用 [GitHub Issues](docs/agents/issue-tracker.md)；状态标签见 [triage 配置](docs/agents/triage-labels.md)。
- 查询根级文件/目录作用、引用关系或清理边界：读 [仓库文件地图](docs/engineering/repository-map.md)。
