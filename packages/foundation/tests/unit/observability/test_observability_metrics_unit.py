...
测试 SimpleGauge 边界值、M.setup 异常等功能.
"""

import pytest
from ditto_foundation import M, ObservabilityConfig, reset_for_testing
from ditto_foundation.observability.metrics import (
    SimpleGauge,
    _create_gauge,
    _MetricsRegistry,
    configure_metrics,
...
class TestSimpleGauge:
    ...
    def test_gauge_set_updates_value(self) -> None:
        """测试 set() 更新值."""
        reset_for_testing()
        ...
        gauge.set(5.0)
        # [REVIEW] callback 读取，无法直接获取
        # [REVIEW]

    def test_gauge_set_negative_value(self) -> None:
        """测试 set() 负数值."""
        reset_for_testing()
        config = ObservabilityConfig(
            pytest_running=True, assertions_enabled=True, verbose_logging=False
    ...
    def test_gauge_inc_increases_value(self) -> None:
        """测试 inc() 增加值."""
        reset_for_testing()
        config = ObservabilityConfig(
            pytest_running=True, assertions_enabled=True, verbose_logging=False
    ...
    def test_gauge_inc_default_delta(self) -> None:
        """测试 inc() 默认增量 1.0."""
        reset_for_testing()
        config = ObservabilityConfig(
            pytest_running=True, assertions_enabled=True, verbose_logging=False
    ...
    def test_gauge_inc_negative_delta(self) -> None:
        """测试 inc() 负数增量(实际减少值)."""
        reset_for_testing()
        config = ObservabilityConfig(
            pytest_running=True, assertions_enabled=True, verbose_logging=False
    ...
    def test_gauge_inc_prevents_negative(self) -> None:
        """测试 inc() 防止值低于 0.0."""
        reset_for_testing()
        config = ObservabilityConfig(
            pytest_running=True, assertions_enabled=True, verbose_logging=False
        ...
        gauge.set(2.0)
        gauge.inc(-5.0)  # [REVIEW] 5.0
        # [REVIEW] 0.0

    def test_gauge_dec_decreases_value(self) -> None:
        """测试 dec() 减少值."""
        reset_for_testing()
        config = ObservabilityConfig(
            pytest_running=True, assertions_enabled=True, verbose_logging=False
    ...
    def test_gauge_dec_default_delta(self) -> None:
        """测试 dec() 默认减量 1.0."""
        reset_for_testing()
        config = ObservabilityConfig(
            pytest_running=True, assertions_enabled=True, verbose_logging=False
    ...
    def test_gauge_dec_prevents_negative(self) -> None:
        """测试 dec() 防止值低于 0.0."""
        reset_for_testing()
        config = ObservabilityConfig(
            pytest_running=True, assertions_enabled=True, verbose_logging=False
        ...
        gauge.set(2.0)
        gauge.dec(5.0)  # [REVIEW] 5.0
        # [REVIEW] 0.0

    def test_gauge_dec_from_zero(self) -> None:
        """测试从 0.0 减少(保持 0.0)."""
        reset_for_testing()
        config = ObservabilityConfig(
            pytest_running=True, assertions_enabled=True, verbose_logging=False
    ...
    def test_gauge_multiple_operations(self) -> None:
        """测试多次操作."""
        reset_for_testing()
        config = ObservabilityConfig(
            pytest_running=True, assertions_enabled=True, verbose_logging=False
...
class TestMSetup:
    ...
    def test_m_setup_unknown_metric_type_raises_error(self) -> None:
        ...
        from ditto_foundation.observability.metrics import METRIC_DEFINITIONS
