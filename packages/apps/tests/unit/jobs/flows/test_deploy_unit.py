...
import pytest
from ditto_apps.jobs.flows.deploy import (
...
class TestListFlows:
    ...
    def test_list_flows_returns_dict(self, mocker: MockerFixture):
        ...
        mock_get_configs = mocker.patch(
            "ditto_apps.jobs.flows.deploy._get_flow_configs"
        )
...
class TestDeployAllFlows:
    ...
    def test_deploy_passes_schedule_to_deployment(self, mocker: MockerFixture):
        ...
        # Arrange
        mocker.patch("ditto_apps.jobs.flows.deploy.deploy")
        mock_get_configs = mocker.patch(
            "ditto_apps.jobs.flows.deploy._get_flow_configs"
        )
...
class TestMain:
    ...
    def test_main_list_command(self, mocker: MockerFixture):
        ...
        mock_list_flows = mocker.patch("ditto_apps.jobs.flows.deploy.list_flows")
        mocker.patch("ditto_apps.jobs.flows.deploy.logger")
        ...
        try:
            # Act
            from ditto_apps.jobs.flows import deploy
...
class TestBackfillDateRange:
    ...
    def test_default_values(self, monkeypatch) -> None:
        """未设置环境变量时应返回默认值."""
        from ditto_apps.jobs.flows.deploy import _get_backfill_date_range
