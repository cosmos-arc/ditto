...
def research_bundle_from_container(container: Container) -> ResearchBundle:
    """Resolve the research surface from one caller-owned DI container."""
...
def create_research_bundle() -> Generator[ResearchBundle]:
    ...
    try:
        yield research_bundle_from_container(container)
    finally:
        container.close()
