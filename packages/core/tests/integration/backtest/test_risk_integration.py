...
def _make_single_loss_data() -> dict[str, pl.DataFrame]:
    """构造触发 SingleLossLimitRule 的价格数据.

    Day 1: ETF-001 = 10.0 (买入价), ETF-002/003 正常
    Day 2: ETF-001 = 7.0 (亏损 30% > 15% 阈值), 触发 REDUCE_POSITION
    Day 3: ETF-001 = 6.5, ETF-002/003 正常

    SingleLossLimitRule 对单个标的生成 action (instrument_id != "*"),
    EngineLoop 会将 ETF-001 加入 risk_locked。
    """
    return {
...
def _make_concentration_data() -> dict[str, pl.DataFrame]:
    """构造触发 ConcentrationLimitRule 的价格数据 (5 日).

    使用 top_k=1:
      Day 1: 信号全为 0 (prev_close == close), 管道选 ETF-001
             大额买单可能被 buying_power reject
      Day 2: ETF-001 信号 > 0, 买单通过 -> 建仓 ETF-001 (~100%)
      Day 3: ConcentrationLimitRule(max_weight=0.20) 检测到 ETF-001 ~100%
             -> REDUCE_POSITION -> ETF-001 锁定
             -> RiskLockFilter 过滤 ETF-001 -> Pipeline 选其他标的
...
def _make_anomaly_data() -> dict[str, pl.DataFrame]:
    """构造触发 MarketAnomalyRule 的价格数据.

    Day 2: ETF-001 日涨跌幅 > 阈值 (close=12.0, prev_close=10.0, +20%)
...
class TestPostTradeRiskLockIntegration:
    ...
    def test_single_loss_triggers_lock_and_prevents_buy(self, tmp_path: Path) -> None:
        ...
        assert "ETF-001" in day1_instruments

        # Day 2: ETF-001 不应有新的买入单
        ...
        day2_buy_fills = [
            f
            for f in day2_fills
            if f.direction.value == "buy" and f.instrument_id == "ETF-001"
        ]
        assert len(day2_buy_fills) == 0, "ETF-001 应被锁定, 不应在 Day 2 产生新的买单"
        ...
        loss_records = [
            r
            for r in risk_log
            if r.rule_id == "single_loss_limit" and r.instrument_id == "ETF-001"
        ]
        assert len(loss_records) > 0, "审计日志应包含 ETF-001 的 single_loss_limit 记录"
    ...
    def test_concentration_limit_triggers_reduce(self, tmp_path: Path) -> None:
        ...
        # ConcentrationLimitRule 触发后, ETF-001 被锁定
        # 后续不应有 ETF-001 的买单 (锁定后 Pipeline 过滤)
        fills = result.fills
        ...
        assert len(post_lock_buys) == 0, (
            f"ETF-001 在 {trigger_date} 被锁定后, 不应有新买单"
        )
    ...
    def test_market_anomaly_creates_audit_records(self, tmp_path: Path) -> None:
        ...
        # 审计日志: Day 2 应有 market_anomaly 记录 (ETF-001 涨幅 20% > 5%)
        risk_log = audit_collector.get_risk_log()
        anomaly_records = [r for r in risk_log if r.rule_id == "market_anomaly"]
        assert len(anomaly_records) > 0, "审计日志应包含 market_anomaly 记录"

        # 应包含 ETF-001 的记录
        etf001_records = [r for r in anomaly_records if r.instrument_id == "ETF-001"]
        assert len(etf001_records) > 0, "审计日志应包含 ETF-001 的 market_anomaly 记录"
