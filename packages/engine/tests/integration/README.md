...
```bash
pixi run -e dev pytest packages/engine/tests/integration -v
```
...
```bash
# 运行回测流程测试
pixi run -e dev pytest packages/engine/tests/integration/test_backtest_flow.py -v

# 运行调仓流程测试
pixi run -e dev pytest packages/engine/tests/integration/test_rebalance_flow.py -v

# 运行风控流程测试
pixi run -e dev pytest packages/engine/tests/integration/test_risk_flow.py -v

# 运行双引擎对齐测试
pixi run -e dev pytest packages/engine/tests/integration/test_engine_alignment.py -v
```
...
```bash
# 只运行集成测试
pixi run -e dev pytest -m integration packages/engine/tests/

# 排除慢速测试
pixi run -e dev pytest -m "integration and not slow" packages/engine/tests/
```
...
def test_hub():
    """创建测试用的 DataHub"""
    return DataHub(data_root="packages/engine/data")
...
def prepare_test_data():
    """准备集成测试所需数据"""
    hub = DataHub(data_root="packages/engine/data")
    ...
    factors.write_parquet("packages/engine/data/factors/expected_factors.parquet")
