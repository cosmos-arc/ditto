...
```bash
pixi run -e dev pytest packages/engine/tests/unit -v
```
...
```bash
# 运行引擎模块测试
pixi run -e dev pytest packages/engine/tests/unit/test_regime*.py -v

# 运行因子模块测试
pixi run -e dev pytest packages/engine/tests/unit/test_*factor*.py -v

# 运行组合管理测试
pixi run -e dev pytest packages/engine/tests/unit/test_portfolio*.py -v

# 运行策略模块测试
pixi run -e dev pytest packages/engine/tests/unit/test_strategy*.py -v
```
...
```bash
# 运行单个测试
pixi run -e dev pytest packages/engine/tests/unit/test_regime_detector.py::test_regime_bull_market -v

# 运行包含特定关键字的测试
pixi run -e dev pytest packages/engine/tests/unit -k "regime" -v
```
...
```bash
# 生成覆盖率报告
pixi run -e dev pytest packages/engine/tests/unit --cov=ditto_engine --cov-report=term-missing

# 生成 HTML 覆盖率报告
pixi run -e dev pytest packages/engine/tests/unit --cov=ditto_engine --cov-report=html
```
