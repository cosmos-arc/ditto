...
# 运行所有测试（带覆盖率）
pixi run -e dev pytest packages/engine/tests --cov=ditto_engine --cov-report=html
```
...
# 运行特定模块单元测试
pixi run -e dev pytest packages/engine/tests/unit/test_regime_engine.py -v

# 运行特定测试函数
pixi run -e dev pytest packages/engine/tests/unit/test_regime_engine.py::test_regime_bull_market -v
```
...
# 查看报告
# 打开 packages/engine/htmlcov/index.html
```

## 测试数据

测试数据存储在 `packages/engine/data/` 目录：
