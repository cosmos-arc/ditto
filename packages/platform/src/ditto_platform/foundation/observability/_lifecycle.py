...
from ._registry import ObservabilityRegistry as _Registry
...
def init(config: ObservabilityConfig, *, force: bool = False) -> None:
    """初始化观测系统（显式配置）。"""
    if _Registry.is_initialized() and not force:
        if _Registry.config and _Registry.config != config:
    ...
    _Registry.set_initialized(True)
