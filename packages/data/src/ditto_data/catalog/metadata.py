    ...
    def __post_init__(self) -> None:
        ...
        for field, value in required_text.items():
            if not value or value.strip() != value:
                msg = f"Invalid product contract {field}: {value!r}"
...
_R2_HARD_SCOPE: frozenset[str] = frozenset(
    {
        "calendar",
        "stock_basic",
        "etf_basic",
...
_APPEND_ONLY_DATASETS: frozenset[str] = frozenset(
    {
        "adj_factor",
        "fund_adj",
        "balance_sheet",
...
_EFFECTIVE_DATED_DATASETS: frozenset[str] = frozenset(
    {
        "stock_basic",
        "etf_basic",
        "index_basic",
...
_KNOWLEDGE_DATE_DATASETS: frozenset[str] = _APPEND_ONLY_DATASETS | frozenset(
...
def _r2_coverage_targets(dataset_id: str) -> tuple[str | None, str | None, str]:
...
def _resolve_product_contract(dataset_id: str) -> DatasetProductContract:
    raw_from, certified_from, coverage_rule = _r2_coverage_targets(dataset_id)
...
def default_dataset_metadata() -> dict[str, DatasetMetadata]:
    ...
    return {
        dataset_id: DatasetMetadata(
            ...
            product_contract=_resolve_product_contract(dataset_id),
        )
