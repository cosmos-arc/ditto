...
# Tushare exchange 到 Ditto exchange 的映射
# Tushare 使用: SZ (深圳), SH (上海), BJ (北京)
# Ditto 使用 ISO 10383 MIC 简化版: XSHE, XSHG, XBSE
_TUSHARE_TO_DITTO: dict[str, str] = {
    "SZ": "XSHE",
    "SH": "XSHG",
    "BJ": "XBSE",
}

_DITTO_TO_TUSHARE: dict[str, str] = {v: k for k, v in _TUSHARE_TO_DITTO.items()}


class TushareExchangeTransformer:
    """
    Tushare 数据源交易所转换器.

    实现 ExchangeTransformer 协议，提供 Tushare 代码与标准格式之间的双向转换.
    ...
    def to_standard(self, source_ticker: str) -> str:
        ...
        Args:
            source_ticker: Tushare 代码 (e.g., "000001.SZ")
        ...
        ditto_exchange = _TUSHARE_TO_DITTO.get(exchange, exchange)
    ...
    def from_standard(self, standard_ticker: str) -> str:
        ...
        tushare_exchange = _DITTO_TO_TUSHARE.get(exchange, exchange)
        return f"{ticker}.{tushare_exchange}"


__all__ = ["TushareExchangeTransformer"]
