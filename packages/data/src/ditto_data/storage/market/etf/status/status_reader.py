...
Provides read-only access to ETF status information stored in Parquet files
with year partitioning. Following design document at docs/design/02_data_design.md.
"""


class EtfStatusReader:
    """
    ETF status data reader.

    Provides read-only access to ETF status data with year partitioning.
    ...
    Attributes:
        DATASET: Dataset name for ETF status.

    """

    DATASET: str = "market/etf/status"

    def __init__(self, data_root: Path) -> None:
        """
        Initialize EtfStatusReader.
