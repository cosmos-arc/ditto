...
class DataStoreSettings(BaseModel):
    ...
    # ========== 市场数据路径（向后兼容，委托至 paths.market）==========

    @property
    def market_stock_bars_path(self) -> Path:
        """股票日线行情路径。"""
        return self.paths.market.stock_bars

    @property
    def market_etf_bars_path(self) -> Path:
        """ETF 日线行情路径。"""
        return self.paths.market.etf_bars

    @property
    def market_index_bars_path(self) -> Path:
        """指数日线行情路径。"""
        return self.paths.market.index_bars

    @property
    def market_stock_status_path(self) -> Path:
        """股票状态路径。"""
        return self.paths.market.stock_status

    @property
    def market_etf_status_path(self) -> Path:
        """ETF 状态路径。"""
        return self.paths.market.etf_status

    @property
    def market_stock_adj_path(self) -> Path:
        """股票复权因子路径。"""
        return self.paths.market.stock_adj

    @property
    def market_etf_adj_path(self) -> Path:
        """ETF 复权因子路径。"""
        return self.paths.market.etf_adj

    @property
    def market_etf_nav_path(self) -> Path:
        """ETF 净值路径。"""
        return self.paths.market.etf_nav

    # ========== 资金流路径（向后兼容，委托至 paths.capital）==========

    @property
    def capital_flow_path(self) -> Path:
        """资金流路径。"""
        return self.paths.capital.flow

    @property
    def capital_margin_path(self) -> Path:
        """融资融券路径。"""
        return self.paths.capital.margin

    @property
    def capital_top_board_path(self) -> Path:
        """龙虎榜路径。"""
        return self.paths.capital.top_board

    @property
    def capital_limit_board_path(self) -> Path:
        """涨跌停路径。"""
        return self.paths.capital.limit_board

    @property
    def capital_chip_path(self) -> Path:
        """筹码分布路径。"""
        return self.paths.capital.chip

    # ========== 基本面路径（向后兼容，委托至 paths.fundamental）==========

    @property
    def fundamental_financial_path(self) -> Path:
        """财务数据路径。"""
        return self.paths.fundamental.financial

    @property
    def fundamental_indicator_path(self) -> Path:
        """财务指标路径。"""
        return self.paths.fundamental.indicator

    @property
    def fundamental_forecast_path(self) -> Path:
        """业绩预告路径。"""
        return self.paths.fundamental.forecast

    @property
    def fundamental_holding_path(self) -> Path:
        """持股数据路径。"""
        return self.paths.fundamental.holding

    # ========== 宏观路径（向后兼容，委托至 paths.macro）==========

    @property
    def macro_indicators_path(self) -> Path:
        """宏观指标路径。"""
        return self.paths.macro.indicators

    # ========== 通用路径（向后兼容，委托至 paths.utility）==========

    @property
    def logs_path(self) -> Path:
        """日志存储路径（支持覆盖）。"""
        return self.paths.utility.logs

    @property
    def backups_path(self) -> Path:
        """备份存储路径。"""
        return self.paths.utility.backups

    @property
    def temp_path(self) -> Path:
        """临时文件存储路径。"""
        return self.paths.utility.temp

    @property
    def db_path(self) -> Path:
        """数据库存储路径。"""
        return self.paths.utility.db
