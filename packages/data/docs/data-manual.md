...
交易日水平值；评价窗口（跟踪与流动性）只从 cutoff 可见的保留日历快照
（`retained_trading_days`）派生，日历证据缺失或未覆盖研究日（允许周末与春节/
国庆长假的自然日桥接）时整个比较 fail closed，不读无版本的当前日历；入选日历
快照身份以 `calendar_snapshot_id` 随跟踪结果一起返回，保证窗口来源可审计可回放，
