...
Relocated from ``ditto_data.services.forward_return_service`` because the
service depends on :class:`MarketService` (data-layer I/O) while its primary
consumer (:class:`FactorEvaluationFacade`) lives in the app query layer.
Placing it here keeps the dependency direction clean — *app → data* — without
forcing analytics to depend on data.
"""
