...
| `fetch_etf_basic()` | ETF 基础信息 | `source_ticker, symbol, name, exchange, list_date` |
| `fetch_etf_daily(date)` | ETF 日线 | OHLCV + `pre_close, amount, pct_change` |
| `fetch_stock_basic()` | 股票基础信息 | `source_ticker, symbol, name, exchange, list_date` |
| `fetch_stock_daily(date)` | 股票日线 | OHLCV + `pre_close, amount, pct_change` |
| `fetch_adj_factor(date)` | 股票复权因子 | `source_ticker, trade_date, adj_factor` |
| `fetch_fund_adj(date)` | ETF/基金复权因子 | `source_ticker, trade_date, adj_factor` |
...
- 实现 7 个 fetch 方法
- pandas → polars 转换
- 字段映射：`ts_code→source_ticker`, `vol→volume`, `pct_chg→pct_change`
