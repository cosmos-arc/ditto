...
class EtfStatusStore(ParquetStoreBase):
    ...
    dataset name "market/etf/status". The read() method does not require a
    dataset parameter.
    """
    ...
    def get_years(self) -> list[int]:
        """
        Get available years for ETF status data.
    ...
    def get_date_range(self) -> tuple[str | None, str | None]:
        """
        Get overall date range for ETF status data.
    ...
    def list_sids(self) -> list[int]:
        """
        List unique security IDs in ETF status data.
