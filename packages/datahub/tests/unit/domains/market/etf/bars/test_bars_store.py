...
@pytest.fixture
def store(data_root: Path) -> EtfBarsStore:
    """Create EtfBarsStore instance."""
    return EtfBarsStore(data_root)


@pytest.fixture
def sample_bars_df() -> pl.DataFrame:
    """Create sample ETF daily bars DataFrame."""
    return pl.DataFrame(
        {
...
class TestEtfBarsStore:
    ...
    def test_read_no_filters(
        self, store: EtfBarsStore, sample_bars_df: pl.DataFrame
    ) -> None:
    ...
    def test_write_new_file(
        ...
        assert result.file_path == str(
            tmp_path / "data" / "market" / "etf" / "bars" / "2024.parquet"
        )
        ...
        new_data = pl.DataFrame(
            {
                ...
                "low": [3.54, 2.88],
                "close": [3.60, 2.92],
                "volume": [950000, 850000],
                "amount": [3420000, 2482000],
            }
        )
    ...
    def test_write_creates_directory(
        ...
        dataset_dir = tmp_path / "data" / "market" / "etf" / "bars"
    ...
    def test_get_years_empty(self, store: EtfBarsStore) -> None:
        """Test get_years with no data."""
        years = store.get_years()
        assert years == []

    def test_get_years(self, store: EtfBarsStore, sample_bars_df: pl.DataFrame) -> None:
    ...
    def test_get_checksum_missing_file(self, store: EtfBarsStore) -> None:
    ...
    def test_count_empty(self, store: EtfBarsStore) -> None:
        """Test count with no data."""
        count = store.count()
        assert count == 0

    def test_count(self, store: EtfBarsStore, sample_bars_df: pl.DataFrame) -> None:
    ...
    def test_get_date_range_empty(self, store: EtfBarsStore) -> None:
    ...
    def test_list_sids_empty(self, store: EtfBarsStore) -> None:
        """Test list_sids with no data."""
        instrument_ids = store.list_sids()
        assert instrument_ids == []

    def test_list_sids(self, store: EtfBarsStore, sample_bars_df: pl.DataFrame) -> None:
