...
def sample_constituents_df() -> pl.DataFrame:
    ...
    return pl.DataFrame(
        {
            "index_sid": [1600001, 1600001, 1600001, 1600001, 1600002],
            "stock_sid": [1, 2, 3, 4, 1],
            "effective_date": [
                date(2024, 1, 1),
                date(2024, 1, 1),
...
def sample_constituents_with_changes() -> pl.DataFrame:
    ...
    return pl.DataFrame(
        {
            "index_sid": [1600001, 1600001, 1600001, 1600001, 1600001, 1600001],
            "stock_sid": [1, 2, 3, 4, 2, 5],
            "effective_date": [
                date(2024, 1, 1),
                date(2024, 1, 1),
                date(2024, 1, 1),
...
class TestIndexConstituentStore:
    ...
    def test_write_empty_df(self, store: IndexConstituentStore) -> None:
        ...
        empty_df = pl.DataFrame(
            {"index_sid": [], "stock_sid": [], "effective_date": [], "weight": []}
        )
    ...
    def test_get_empty(self, store: IndexConstituentStore) -> None:
        ...
        result = store.get(index_sid=1600001, asof=date(2024, 1, 1))
        assert len(result) == 0
    ...
    def test_get_simple(
        ...
        result = store.get(index_sid=1600001, asof=date(2024, 1, 1))
        assert len(result) == 4
        assert set(result["stock_sid"].to_list()) == {1, 2, 3, 4}
    ...
    def test_get_with_date_before_effective(
        ...
        store.write(sample_constituents_df)
        result = store.get(index_sid=1600001, asof=date(2023, 12, 31))
        assert len(result) == 0
    ...
    def test_get_pit(
        ...
        # Before change
        result = store.get(index_sid=1600001, asof=date(2024, 1, 15))
        assert len(result) == 4
        assert set(result["stock_sid"].to_list()) == {1, 2, 3, 4}

        # After change (stock 2 removed, stock 5 added)
        result = store.get(index_sid=1600001, asof=date(2024, 2, 15))
        # Should have stocks 1, 3, 4, 5 (stock 2 removed with weight 0)
        constituents = result.filter(pl.col("weight") > 0)
        assert set(constituents["stock_sid"].to_list()) == {1, 3, 4, 5}
    ...
    def test_get_multiple_indices(
        ...
        result1 = store.get(index_sid=1600001, asof=date(2024, 1, 1))
        assert len(result1) == 4

        result2 = store.get(index_sid=1600002, asof=date(2024, 1, 1))
        assert len(result2) == 1
        assert result2["stock_sid"][0] == 1
    ...
    def test_get_nonexistent_index(
        ...
        store.write(sample_constituents_df)
        result = store.get(index_sid=1699999, asof=date(2024, 1, 1))
        assert len(result) == 0
    ...
    def test_read_filter_by_index_sid(
        ...
        """Test read filtered by index_sid."""
        store.write(sample_constituents_df)
        result = store.read(index_sids=[1600001])
        assert len(result) == 4
        assert all(result["index_sid"] == 1600001)
    ...
    def test_delete_by_index_sid(
        ...
        deleted_count = store.delete(index_sids=[1600001])
        assert deleted_count == 4

        # Verify deletion
        result = store.read()
        assert len(result) == 1  # Only 1600002 remains
        assert result["index_sid"][0] == 1600002
    ...
    def test_delete_by_date_range(
        ...
        deleted_count = store.delete(
            index_sids=[1600001], start_date="2024-01-01", end_date="2024-01-01"
        )
        assert deleted_count == 4
    ...
    def test_count_filter_by_index(
        ...
        store.write(sample_constituents_df)
        count = store.count(index_sids=[1600001])
        assert count == 4
