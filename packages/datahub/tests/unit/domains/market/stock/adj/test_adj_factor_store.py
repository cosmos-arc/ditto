...
@pytest.fixture
def store(data_root: Path) -> StockAdjFactorStore:
    """Create StockAdjFactorStore instance."""
    return StockAdjFactorStore(data_root)
    ...
    return pl.DataFrame(
        {
            ...
            "adj_factor": [1.0, 0.95, 0.95, 1.0],
        }
    )


class TestStockAdjFactorStore:
    """Test suite for StockAdjFactorStore."""

    # ============ _get_key_columns tests ============

    def test_get_key_columns(self, store: StockAdjFactorStore) -> None:
        """Test _get_key_columns returns correct key columns."""
        assert store._get_key_columns() == ["sid", "trade_date"]

    # ============ read tests ============

    def test_read_empty(self, store: StockAdjFactorStore) -> None:
        """Test read with no data."""
        df = store.read()
        assert len(df) == 0

    def test_read_no_filters(
        self, store: StockAdjFactorStore, sample_adj_factor_df: pl.DataFrame
    ) -> None:
    ...
    def test_write_new_file(
        self,
        store: StockAdjFactorStore,
        sample_adj_factor_df: pl.DataFrame,
        tmp_path: Path,
    ) -> None:
        """Test write creates new file."""
        result = store.write(sample_adj_factor_df, year=2024)

        assert result.file_path == str(
            tmp_path / "data" / "market" / "stock" / "adj" / "2024.parquet"
        )
    ...
    def test_write_creates_directory(
        ...
        dataset_dir = tmp_path / "data" / "market" / "stock" / "adj"
    ...
    def test_get_years_empty(self, store: StockAdjFactorStore) -> None:
    ...
    def test_get_years(
        ...
        store.write(sample_adj_factor_df, year=2022)
        store.write(sample_adj_factor_df, year=2024)
        store.write(sample_adj_factor_df, year=2023)
    ...
    def test_delete_year(
        ...
        file_path = tmp_path / "data" / "market" / "stock" / "adj" / "2024.parquet"
        assert not file_path.exists()

    def test_delete_nonexistent_year(self, store: StockAdjFactorStore) -> None:
    ...
    def test_get_checksum_missing_file(self, store: StockAdjFactorStore) -> None:
    ...
    def test_count_empty(self, store: StockAdjFactorStore) -> None:
    ...
    def test_count_with_filters(
        ...
        store.write(sample_adj_factor_df, year=2024)
        count = store.count(sids=[1000001])
        assert count == 3

    # ============ get_date_range tests ============

    def test_get_date_range_empty(self, store: StockAdjFactorStore) -> None:
    ...
    def test_list_sids_empty(self, store: StockAdjFactorStore) -> None:
    ...
    def test_dataset_name(self, store: StockAdjFactorStore) -> None:
        """Test that dataset name is stock_adj_factor."""
        assert store._dataset == "market/stock/adj"
