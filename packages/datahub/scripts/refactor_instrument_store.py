...
def refactor_instrument_store(content: str) -> str:
    ...
    命名映射规则：
    - Python API: resolve_sid -> resolve_instrument_id
    - Python API: src_code -> source_ticker
    - 数据库 SQL: 保持 sid, src_code, security, security_mapping（不变）
    """
    # 1. 方法名替换：resolve_sid -> resolve_instrument_id
    # 但保留 SQL 查询中的 sid 列名
    content = re.sub(
        r"def resolve_sid\(",
    ...
    # 2. 方法名替换：resolve_sids_batch -> resolve_instrument_ids_batch
    content = re.sub(
        r"def resolve_sids_batch\(",
        "def resolve_instrument_ids_batch(",
        content,
    )

    # 3. 参数名替换：src_code -> source_ticker（仅函数签名和文档字符串）
    # 注意：SQL 查询中的 src_code 必须保持不变

    # 4. 更新文档字符串中的描述
    content = re.sub(
        r"Resolve src_code to sid",
        "Resolve source_ticker to instrument_id",
        content,
    )

    content = re.sub(
        r"Resolve source code to sid",
        "Resolve source ticker to instrument_id",
        content,
    )

    content = re.sub(
        r"Batch resolve src_codes to sids",
        "Batch resolve source_tickers to instrument_ids",
        content,
    )

    # 5. 更新变量名和日志消息（仅 Python 代码，不涉及 SQL）
    # 这个需要更精细的处理，因为 src_code 在 SQL 中必须保留
