...
class TestInitConfigCommand:
    ...
    def test_init_config_uses_default_data_root(self, mocker, mock_ctx):
        """测试使用默认数据根目录。"""
        # Arrange
    ...
    def test_init_config_with_custom_data_root(self, mocker, mock_ctx):
        """测试指定自定义数据根目录。"""
        # Arrange
    ...
    def test_init_config_with_force_flag(self, mocker, mock_ctx):
        """测试强制重新初始化。"""
        # Arrange
    ...
    def test_init_config_handles_failure(self, mocker, mock_ctx):
        """测试初始化失败时正确处理。"""
        # Arrange
        mocker.patch("ditto_port.cli.commands.init.register_datahub_providers")
        ...
        mock_coordinator = mocker.MagicMock()
        mock_coordinator.initialize.return_value = {
            "dq_config": MagicMock(success=False, skipped=False, message="初始化失败"),
        }
        mocker.patch(
            "ditto_port.cli.commands.init.get_config_coordinator",
            return_value=mock_coordinator,
...
class TestInitDQCommand:
    ...
    def test_init_dq_filters_results(self, mocker, mock_ctx):
        """测试只显示 DQ 相关结果。"""
        # Arrange
        mocker.patch("ditto_port.cli.commands.init.register_datahub_providers")
...
class TestInitDBCommand:
    ...
    def test_init_db_filters_results(self, mocker, mock_ctx):
        """测试只显示数据库相关结果。"""
        # Arrange
        mocker.patch("ditto_port.cli.commands.init.register_datahub_providers")
        ...
        mock_coordinator = mocker.MagicMock()
        mock_coordinator.initialize.return_value = {
            "dq_config": MagicMock(success=True, skipped=False, message="DQ配置"),
            "database_schema": MagicMock(
                success=True, skipped=False, message="数据库Schema已初始化"
            ),
        }
