...
class TestDqBatchCheckKnownErrorHandling:
    ...
    async def test_known_error_records_failure_in_results(
        ...
        mocker.patch(
            "ditto_port.jobs.tasks.dq_batch.create_dq_and_metadata_context",
            return_value=mock_context,
        )
        mocker.patch(
            "ditto_port.jobs.tasks.dq_batch.L3BatchService",
            return_value=mock_l3_service,
        )
        # Mock _send_dq_alert 避免副作用
        mocker.patch(
            "ditto_port.jobs.tasks.dq_batch._send_dq_alert",
            return_value=None,
        )
