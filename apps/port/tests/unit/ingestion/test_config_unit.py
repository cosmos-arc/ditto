...
import pytest
from ditto_port.services.ingestion.config import IngestionConfig
...
def test_config_from_env(monkeypatch) -> None:
    ...
    monkeypatch.setenv("DITTO_DATA_DIR", "/tmp/data")  # noqa: S108 - test data path
    monkeypatch.setenv("DITTO_DEFAULT_SOURCE", "akshare")
    monkeypatch.setenv("DITTO_AUTO_REGISTER_SECURITIES", "false")

    config = IngestionConfig()

    assert config.data_root == Path("/tmp/data")  # noqa: S108 - test data path
