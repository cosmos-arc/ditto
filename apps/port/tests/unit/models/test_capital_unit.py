...
MarginQuery, Margin, ValuationQuery, Valuation, FuturesQuery, Futures.
"""
...
class TestFuturesQuery:
    """测试 FuturesQuery 查询参数模型."""

    def test_required_fields(self) -> None:
        """验证必填字段."""
        from ditto_port.models.capital import FuturesQuery

        query = FuturesQuery(instrument_id="IF2401", as_of_date=date(2024, 1, 15))
        assert query.instrument_id == "IF2401"
        assert query.as_of_date == date(2024, 1, 15)
...
class TestFutures:
    ...
    def test_basic_futures(self) -> None:
        """验证基本 Futures 创建."""
        from ditto_port.models.capital import Futures
        ...
        assert futures.instrument_id == "IF2401"
        assert futures.trade_date == "2024-01-15"
        assert futures.open_interest == 100000
        assert futures.settlement_price == 3850.0
        assert futures.volume == 50000
        assert futures.turnover == 192500000.0

    def test_futures_with_optional_fields_none(self) -> None:
        """验证可选字段为 None."""
        from ditto_port.models.capital import Futures
        ...
        assert futures.settlement_price is None
        assert futures.turnover is None
...
class TestToFutures:
    ...
    def test_convert_complete_row(self) -> None:
        """验证完整行转换."""
        from ditto_port.models.capital import to_futures
...
@pytest.mark.unit
class TestToFuturesList:
    """测试 to_futures_list 转换函数."""

    def test_convert_empty_dataframe(self) -> None:
        """验证空 DataFrame 转换."""
        from ditto_port.models.capital import to_futures_list

        df = pl.DataFrame()
        result = to_futures_list(df)
        assert result == []
    ...
    def test_convert_multiple_rows_dataframe(self) -> None:
        ...
        df = pl.DataFrame(
            {
                "instrument_id": ["IF2401", "IF2401"],
                "trade_date": ["2024-01-15", "2024-01-16"],
                "open_interest": [100000, 110000],
                "settlement_price": [3850.0, 3860.0],
                "volume": [50000, 55000],
                "turnover": [192500000.0, 212300000.0],
            }
        )
        ...
        assert len(result) == 2
        assert result[0].open_interest == 100000
        assert result[1].open_interest == 110000
