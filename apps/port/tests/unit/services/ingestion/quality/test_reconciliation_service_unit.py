...
@pytest.fixture
def sync_comparison_accessor(mock_comparison_accessor):
    ...
    mock_comparison_accessor.write_result.side_effect = write_result_impl
    return mock_comparison_accessor
...
class TestDailyReconciliationSuccess:
    ...
    async def test_no_issues_no_storage(
        ...
        # Assert - 验证不存储结果
        mock_comparison_accessor.write_result.assert_not_called()
        assert result["passed"] is True
...
class TestDailyReconciliationWithIssues:
    ...
    async def test_with_issues_stores_result(
        ...
        # 验证存储结果被调用
        mock_comparison_accessor.write_result.assert_called_once()
