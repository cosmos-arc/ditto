...
async def get_financials(
    ...
    if report_type == FinancialType.BALANCE_SHEET:
        df = service.get_balance_sheet(instrument_id, as_of_date)
    elif report_type == FinancialType.INCOME_STATEMENT:
        df = service.get_income_statement(instrument_id, as_of_date)
    elif report_type == FinancialType.CASH_FLOW:
        df = service.get_cash_flow(instrument_id, as_of_date)
...
async def get_dividend(
    ...
    """
    df = service.get_dividend(instrument_id, as_of_date)
...
async def list_corporate_actions(
    ...
    # 调用 service
    df = service.list_corporate_actions(instrument_id, start_date, end_date)
