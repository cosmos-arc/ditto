"""FX (外汇) 域 API 路由."""

import asyncio
from typing import Annotated

import polars as pl
from dishka import FromComponent
from dishka.integrations.fastapi import inject
from ditto_datahub.services.market_service import MarketService
from ditto_datahub.sources.tushare.adapters.fx import FX_CODE_TO_INSTRUMENT_ID
from fastapi import APIRouter

from ditto_port.models.common import APIResponse
from ditto_port.models.fx import FxBar, FxQuery, to_fx_bar_list

router = APIRouter(prefix="/fx", tags=["fx"])

# 反向映射：instrument_id -> pair (e.g., 4000001 -> "USDCNH.FXCM")
_INSTRUMENT_ID_TO_PAIR = {v: k for k, v in FX_CODE_TO_INSTRUMENT_ID.items()}


@router.post("/bars", response_model=APIResponse[list[FxBar]])
@inject
async def post_bars(
    query: FxQuery,
    market_service: Annotated[MarketService, FromComponent()],
) -> APIResponse[list[FxBar]]:
    """
    查询外汇 K 线数据.

    Args:
        query: 查询参数
            - pairs: 货币对列表 (可选, 如 ["USDCNH.FXCM", "EURUSD.FXCM"])
            - start_date: 开始日期 (可选)
            - end_date: 结束日期 (可选)
            - limit: 返回数量限制 (默认 1000)
        market_service: MarketService 依赖注入

    Returns:
        APIResponse 包含外汇 K 线数据列表

    """
    # 获取 instrument_ids
    if query.pairs:
        instrument_ids = [
            FX_CODE_TO_INSTRUMENT_ID[pair]
            for pair in query.pairs
            if pair in FX_CODE_TO_INSTRUMENT_ID
        ]
    else:
        instrument_ids = list(FX_CODE_TO_INSTRUMENT_ID.values())

    if not instrument_ids:
        return APIResponse(data=[])

    # 构建查询参数
    start_str = query.start_date.isoformat() if query.start_date else None
    end_str = query.end_date.isoformat() if query.end_date else None

    # 查询数据（在线程池中执行，避免阻塞事件循环）
    df = await asyncio.to_thread(
        market_service.list_bars,
        instrument_ids=instrument_ids,
        start=start_str,
        end=end_str,
    )

    # 如果 DataFrame 为空，直接返回空列表
    if df.is_empty():
        return APIResponse(data=[])

    # 添加 pair 列（从 instrument_id 映射）
    df = df.with_columns(
        pl.col("instrument_id").replace(_INSTRUMENT_ID_TO_PAIR).alias("pair")
    )

    # 重命名 trade_date_utc 列以匹配模型
    df = df.select(
        "pair",
        pl.col("trade_date_utc").dt.date().cast(pl.String).alias("trade_date_utc"),
        "open",
        "high",
        "low",
        "close",
    )

    # 转换为模型列表
    bars = to_fx_bar_list(df)

    # 应用 limit
    bars = bars[: query.limit]

    return APIResponse(data=bars)
