...
def build_certification_checks(
    *,
    ...
    common_checks = (
        _build_minimal_dq_check(minimal_dq_record),
        _build_manifest_check(manifest),
    )
    ...
    checks = [
        *common_checks,
        _build_shadow_ready_gate_check(common_checks),
        *_build_diff_or_audit_checks(
...
def _build_shadow_ready_gate_check(
...
def _build_diff_or_audit_checks(
    *,
    materialization_profile: MaterializationProfile,
    shadow_report_record: ShadowDiffReportRecord | None,
) -> tuple[CertificationCheckResult, ...]:
    if materialization_profile == MaterializationProfile.OFFLINE:
        return (_build_sample_audit_check(shadow_report_record),)
    return (_build_shadow_diff_check(shadow_report_record),)
...
def _build_role_checks(
    *,
    ...
    if role == DerivedRole.FACTOR:
        return (_build_factor_distribution_check(shadow_report_record),)
    if role == DerivedRole.FEATURE:
        return (_build_feature_parity_check(shadow_report_record),)
    return ()
...
def _build_profile_checks(
    *,
    ...
    if materialization_profile == MaterializationProfile.SERIES:
        return (_build_series_shadow_parity_check(shadow_report_record),)
    if materialization_profile == MaterializationProfile.OFFLINE:
        return (_build_offline_reproducibility_check(minimal_dq_record),)
    return ()


def _build_shadow_diff_check(
    shadow_report_record: ShadowDiffReportRecord | None,
) -> CertificationCheckResult:
    passed = _shadow_report_passed(shadow_report_record)
    return CertificationCheckResult(
        ...
        metric_value=_shadow_report_error_count(shadow_report_record),
        threshold_value=0,
    )


def _build_sample_audit_check(
    shadow_report_record: ShadowDiffReportRecord | None,
) -> CertificationCheckResult:
    passed = _shadow_report_passed(shadow_report_record)
    return CertificationCheckResult(
...
def _build_factor_distribution_check(
    shadow_report_record: ShadowDiffReportRecord | None,
) -> CertificationCheckResult:
    sample_count = _shadow_report_metric_int(shadow_report_record, "sample_count")
...
def _build_feature_parity_check(
    shadow_report_record: ShadowDiffReportRecord | None,
) -> CertificationCheckResult:
    passed = _shadow_report_passed(shadow_report_record)
    return CertificationCheckResult(
...
def _build_series_shadow_parity_check(
    shadow_report_record: ShadowDiffReportRecord | None,
) -> CertificationCheckResult:
    passed = _shadow_report_passed(shadow_report_record)
    return CertificationCheckResult(
...
def _build_offline_reproducibility_check(
    minimal_dq_record: DerivedMinimalDQSummaryRecord | None,
) -> CertificationCheckResult:
    passed = minimal_dq_record is not None and minimal_dq_record.passed
...
def _shadow_report_passed(shadow_report_record: ShadowDiffReportRecord | None) -> bool:
    return shadow_report_record is not None and shadow_report_record.error_count == 0


def _shadow_report_metric_int(
    shadow_report_record: ShadowDiffReportRecord | None,
    key: str,
) -> int:
...
def _shadow_report_error_count(
    shadow_report_record: ShadowDiffReportRecord | None,
) -> int:
    if shadow_report_record is None:
        return 0
