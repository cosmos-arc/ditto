...
> 全流程审查的详细步骤。只读视角确实独立且范围足够大时，才使用宿主原生并行能力。
...
2. 识别当前最低分维度和天花板维度
3. 从策略矩阵选择本轮创意策略
4. 轻量标杆调研（网页检索 1-2 个参考）
5. 输出本轮创意蓝图（策略/区域/参考/预期/约束）

---

## Phase 3: MULTI-LENS REVIEW（多视角审查）

按变更范围从 [roles.md](roles.md) 选择必要视角。完整跨页评审可覆盖全部视角；局部变更不强制固定数量。

| 角色 | 产出 |
|------|------|
| Art Director | 气质问题清单 + 评分卡 |
| UI Designer | UI 问题清单 |
| UX Reviewer | UX 问题清单 |
| Product Mgr | spec 合规/层级验证/边界守卫 |
| IA Specialist | 信息架构 + 流程问题 |
| Copy Editor | 文案问题清单 |
| Data Viz Specialist | 数据可视化 + 色觉无障碍 |
...
当 manifest 存在时，每个已选择视角都读取跨页一致性基线报告。
...
当状态覆盖率报告存在时，每个已选择视角追加覆盖检查清单。

---

## Phase 4: CONFLICT RESOLUTION（冲突协调）
...
## Phase 7: POLISH（质量提升 + Art Director 审批）

### Step 7.1: Art Director 预审
...
质量等级对应的可观察标准见 [quality-levels.md](quality-levels.md)。

### Step 7.3: Art Director 复审

- 可降级过度强调、装饰和动效
- 可移除违反克制度的装饰元素
- 通过降低对比度、减少层级和移除非必要动效处理过度装饰
- 输出气质评分卡

### Step 7.4: REFLECT [--iterate]

详见 [iterate.md](iterate.md) §每轮反思记录。

---

## Phase 8: FINAL（最终验证 + 气质评分）

### Step 8.0: PRE-SCORE GATES（评分前置门禁）
...
### Step 8.1: 零 Inline Style 门禁

- grep 所有 style="..." 属性（排除 CSS 注释）
- 命中数 > 0 → P0 级阻断，不进入后续步骤

### Step 8.2: Lighthouse 审计
...
### Step 8.5: VP-STANDARD 完整性验证

- 内容无截断，底部元素完全可见
- sticky 元素（rail/header/context-bar）正常工作

### Step 8.6: VP-COMPACT 完整性验证
