	...
	"fixed z-50 gap-4 border border-(--color-border-subtle) bg-(--color-surface-3) shadow-lg transition ease-in-out",
	{
		variants: {
			side: {
				...
				left: "inset-y-0 left-0 h-full w-full max-w-full border-r data-[state=closed]:slide-out-to-left data-[state=open]:slide-in-from-left sm:w-(--width-drawer) sm:max-w-(--width-drawer)",
				right:
					"inset-y-0 right-0 h-full w-full max-w-full border-l data-[state=closed]:slide-out-to-right data-[state=open]:slide-in-from-right sm:w-(--width-drawer) sm:max-w-(--width-drawer)",
			},
		},
		defaultVariants: {
			side: "right",
