...
afterEach(() => {
	window.history.replaceState({}, "", "/portfolio/model");
});

describe("useFillLedger execution scope", () => {
	it("uses the URL strategy for both the request and query cache key", async () => {
		window.history.replaceState({}, "", "/portfolio/transactions?strategy_id=strategy-r1");
		let requestedStrategy: string | null = null;
		server.use(
			http.get("/api/v1/manual/fills", ({ request }) => {
