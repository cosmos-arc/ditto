...
function mapOrderStatus(status: string): GetOrdersResponse["items"][number]["status"] {
	if (status === "filled") return "filled";
	if (status === "partially_filled") return "partial";
	if (status === "cancelled") return "cancelled";
	if (status === "expired" || status === "superseded") return "failed";
	return "pending";
}

function mapV2ActionToOrder(
	report: DailyDecisionV2Response,
	action: DailyDecisionActionResponse,
): GetOrdersResponse["items"][number] | null {
	const direction = normalizeDirection(action.direction);
	if (direction !== "buy" && direction !== "sell") return null;
	const quantity = action.suggested_quantity ?? action.rounded_quantity ?? action.raw_quantity ?? 0;
	const decisionDate =
		report.identity.decision_date ?? report.identity.intended_trade_date ?? report.identity.signal_date ?? "";
	const observedAt = decisionDate ? `${decisionDate}T00:00:00Z` : "";

	return {
		id: action.intent_id,
		instrument: instrumentLabel(action.instrument_id),
		side: direction === "sell" ? "SELL" : "BUY",
		qty: quantity,
		price: action.reference_price ?? 0,
		filledQty: action.filled_quantity ?? 0,
		type: "LIMIT",
		status: mapOrderStatus(action.intent_status ?? "pending"),
		account: report.identity.account_id ?? "未绑定账户",
		createdAt: observedAt,
		updatedAt: observedAt,
	};
}

export function mapDailyDecisionV2ToOrdersResponse(
	report: DailyDecisionV2Response,
	params?: GetOrdersRequest,
): GetOrdersResponse {
	const page = params?.page ?? 1;
	const pageSize = params?.limit ?? params?.pageSize ?? 20;
	const orders = report.actions
		.map((action) => mapV2ActionToOrder(report, action))
		.filter((order): order is GetOrdersResponse["items"][number] => order !== null)
		.filter((order) => !params?.tab || order.status === params.tab);
	const start = (page - 1) * pageSize;
...
export function mapDailyDecisionV2ToOrderDetail(
	report: DailyDecisionV2Response,
	intentId: string,
): GetOrderDetailResponse {
	const action = report.actions.find((candidate) => candidate.intent_id === intentId);
	const order = action ? mapV2ActionToOrder(report, action) : null;
	const fallbackObservedAt = report.identity.decision_date ?? report.identity.intended_trade_date ?? "";
	...
	return {
		order: order ?? {
			...
			status: "failed",
			account: report.identity.account_id ?? "未绑定账户",
			createdAt: fallbackObservedAt,
			updatedAt: fallbackObservedAt,
		},
		trace: [
			{
				time: order?.createdAt ?? fallbackObservedAt,
				event: "决策快照",
				detail: `${report.identity.strategy_id} · ${report.readiness.status}`,
			},
			{
				time: order?.updatedAt ?? fallbackObservedAt,
				event: "人工复核",
				detail: report.readiness.reason_codes.join(" · ") || "后端未返回额外复核原因",
			},
		],
		fees: 0,
		slippage: 0,
		routeLog: [
			{
				time: order?.updatedAt ?? fallbackObservedAt,
				event: "执行模式",
				detail: "manual / paper；没有 Paper order/fill lineage，不展示为已执行。",
			},
		],
	};
}
