...
function useLedgerHandlers(options: {
	...
	server.use(
		http.get("/api/v1/manual/fills", () => HttpResponse.json({ data: raw })),
		http.get("/api/v1/manual/fills/effective", () => HttpResponse.json({ data: effective })),
		http.get("/api/v1/manual/fill-adjustments", () => HttpResponse.json({ data: adjustments })),
	);
}
...
describe("append-only fill correction ledger", () => {
	...
	it("keeps the sheet open and explains backend correction conflicts", async () => {
		useLedgerHandlers({});
		server.use(
			http.post("/api/v1/manual/fills/:fillId/void", () =>
				HttpResponse.json(
					{
