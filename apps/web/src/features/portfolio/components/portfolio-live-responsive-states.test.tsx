...
describe("R1 trading live responsive states", () => {
	it("announces the fill ledger loading state", () => {
		server.use(
			http.get("/api/v1/manual/fills", async () => {
	...
	it("announces the signal queue loading state", () => {
		server.use(
			http.get("/api/v1/manual/daily-decision/v2", async () => {
				await delay("infinite");
				return HttpResponse.error();
			}),
		);

		render(<PortfolioOverviewSignalsPanel />, { wrapper: createWrapper() });
	...
	it("announces the order panel loading state", () => {
		...
		render(<PortfolioOverviewOrdersPanel />, { wrapper: createWrapper() });
	...
	it("announces an empty fill ledger as a completed status", async () => {
		server.use(
			...
			http.get("/api/v1/manual/fills/effective", () => HttpResponse.json({ data: [] })),
			http.get("/api/v1/manual/fill-adjustments", () => HttpResponse.json({ data: [] })),
		);
	...
	it("stacks the pipeline summary on mobile and recovers from a failed source", async () => {
		server.use(
			http.get("/api/v1/manual/fills", () => HttpResponse.json({ detail: "temporary failure" }, { status: 503 }), {
				once: true,
			}),
		);
		const user = userEvent.setup();
