...
import { liveDailyDecisionV3, portfolioHandlers } from "@/mocks/handlers/portfolio";
...
describe("R4 live pages", () => {
	beforeEach(() => {
		...
		window.history.replaceState({}, "", "/portfolio/model?strategy_id=seed_etf_industry_rotation&account_id=paper-a");
	...
	it("renders the R4 cockpit from V3 even when ancillary V2 views fail", async () => {
		...
		expect(analysis?.parentElement).toHaveClass("[--height-analysis-band:var(--height-portfolio-analysis-band)]");
