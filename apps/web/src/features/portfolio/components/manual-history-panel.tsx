...
export function ManualHistoryPanel({
	...
	const historyQuery = useQuery({
		queryKey: tradingKeys.manualHistory(accountId, identity ?? FALLBACK_IDENTITY),
		queryFn: () => fetchManualAccountHistory(accountId, identity ?? FALLBACK_IDENTITY),
		...
		>
			<header className="border-b border-(--color-border-subtle) px-4 py-3">
				...
				<p className="mt-1 text-[11px] text-(--color-foreground-tertiary)">
					只读重放：账本修订 {ledgerRevision.event_count} 条事件；价格来自显式保留快照，不回退 latest
				</p>
			</header>
