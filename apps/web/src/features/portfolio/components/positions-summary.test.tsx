...
describe("PositionsSummary", () => {
	...
	it("盈亏正值显示绿色", async () => {
		...
		const positiveCells = screen
			.getAllByText(/\+?8,300/)
			.filter((el) => el.classList.contains("text-(--color-market-up)"));
	...
	it("data-slot 为 positions-summary", async () => {
		...
		const wrapper = screen.getByText("平安银行").closest('[data-slot="positions-summary"]');
	...
	it("sparkline 颜色根据盈亏方向决定", async () => {
		...
		const upSparklines = document.querySelectorAll('[data-slot="sparkline"][data-variant="up"]');
	...
	it("汇总行显示总盈亏", async () => {
		...
		const summary = document.querySelector('[data-slot="positions-summary-footer"]');
