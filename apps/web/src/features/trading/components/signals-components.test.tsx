...
import { liveDailyDecisionV2, tradingHandlers } from "@/mocks/handlers/trading";
...
describe("SignalDetailPanel", () => {
	...
	it("明确 400 失败允许关闭且不会要求未知结果重试", async () => {
		vi.stubEnv("VITE_USE_MOCK", "false");
		server.use(http.post("/api/v1/trade/fills", () => HttpResponse.json({ detail: "成交价格非法" }, { status: 400 })));
		const user = userEvent.setup();
		render(<SignalDetailPanel signalId="intent-510300" />, { wrapper: createWrapper() });
