...
export function SignalsHealthStrip() {
	...
	if (isLoading) {
		return (
			<div className="grid grid-cols-2 gap-3 px-4 py-2 sm:grid-cols-4">
	...
	if (isError) {
		return (
			<div
				role="alert"
				className="mx-4 my-2 flex flex-col items-start gap-2 rounded-(--radius-sm) border border-(--color-risk-critical-fg) px-3 py-2 text-sm text-(--color-foreground-secondary) sm:flex-row sm:items-center sm:justify-between"
			>
