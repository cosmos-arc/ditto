	...
	return (
		...
		>
			...
			>
				...
				<Separator />
				<span className="text-xs text-(--color-foreground-secondary)">{data?.pendingApprovals ?? "—"} 项待审批</span>
				<Separator />
				<span className="text-xs text-(--color-foreground-secondary)">
					{data?.activeCopilotSessions ?? "—"} 个 Copilot 会话
				</span>
				<div className="flex-1" />
				<a href="/ai/agents" className="text-xs text-(--color-accent) hover:underline">
					查看全部
				</a>
			</div>
		</DittoErrorBoundary>
