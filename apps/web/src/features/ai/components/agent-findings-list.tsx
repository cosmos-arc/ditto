...
const FINDING_STATUS_VARIANT_MAP: Record<string, "warning" | "healthy" | "error"> = {
	pending: "warning",
	approved: "healthy",
	rejected: "error",
};
						...
						{data.items.map((finding) => (
							...
							>
								...
								<div className="mt-2 flex items-center gap-2 text-xs text-(--color-foreground-tertiary)">
									...
									/>
									<StatusBadge
										variant={FINDING_STATUS_VARIANT_MAP[finding.status] ?? "default"}
										label={FINDING_STATUS_LABEL_MAP[finding.status] ?? finding.status}
										size="sm"
									/>
								</div>
