							...
							<PanelBody className="p-3">
								<div className="flex flex-col gap-1">
									{MOCK_TASKS.map((task) => (
										...
										>
											<span
												className={`text-xs ${task.status === "running" ? "text-(--color-system-healthy-fg)" : task.status === "waiting" ? "text-(--color-foreground-muted)" : task.status === "progress" ? "text-(--color-system-degraded-fg)" : "text-(--color-system-down-fg)"}`}
											>
											...
											<span
												className={`shrink-0 text-xs ${task.status === "failed" ? "text-(--color-system-down-fg)" : "text-(--color-foreground-muted)"}`}
											>
												{task.detail}
											</span>
										</div>
									...
									{MOCK_RESOURCES.map((res) => (
										<div key={res.label} className="flex flex-col gap-1">
											<div className="flex items-center justify-between">
												...
												<span
													className={`text-xs tabular-nums ${res.severity === "critical" ? "text-(--color-risk-critical-fg)" : "text-(--color-system-healthy-fg)"}`}
												>
													{res.value}
												</span>
											</div>
								...
								<div className="flex flex-col gap-1">
									{MOCK_EVENTS.map((evt) => (
										...
										>
											...
											<span className="min-w-0 flex-1 truncate text-xs text-(--color-foreground-secondary)">
												{evt.text}
											</span>
										</div>
									))}
