...
## 八、估值与收益合同补充（2026-09-20）

依据：[确定估值与收益合同：ETF 价格、NAV 和三组合历史表现](https://github.com/cosmos-arc/ditto/issues/246)。本节优先于上文涉及收益口径的概括；首期单时点 drift 的已验收范围不变。

1. ETF 原始成交价用于成交模拟、成本和账户持仓估值；含分红拆分的总回报序列用于长期表现及策略研究；单位 NAV 用于基金资产价值、跟踪表现和适用的折溢价分析。三者不可互换，基准须标明价格或总回报口径。
2. 跨境价格与 NAV 不同时点时，只可展示“当前价格相对最近已公布 NAV 的差异”，同时展示价格时间、NAV 估值日及披露时间；不得标为实时溢价或可执行套利信号。IOPV 单独标明来源和时间，不等同真实 NAV。
3. 三组合默认比较排除外部资金进出的时间加权收益（TWR），从共同有效起点归一化；资产金额与绝对盈亏分开。分红为投资收益、费用影响表现、入金不算盈利。Model 使用历史目标与明确成本假设，Paper 使用模拟成交和费用，Manual 使用实际记录；禁止用今天的目标倒填 Model 历史。
4. 历史不足则缩短共同区间；必要估值缺失显示断口或不可比，不插值、不静默沿用旧价。停牌仅按显式估值政策处理，并标记陈旧价格。

现金流时点、子区间估值、零资产及资金耗尽后的重新起算规则须在后续数值合同中明确。没有流入流出时点估值时，不得把未标注的近似计算称为精确 TWR。
