...
from ditto_apps.cli.commands.agent_payloads import (
    approval_payload,
    campaign_payload,
    event_payload,
    run_payload,
...
def _emit_campaign(campaign: CampaignView, *, json_output: bool) -> None:
    if json_output:
        _emit_json(campaign_payload(campaign))
        return
    typer.echo(
        " ".join(
            (
...
def _emit_event(
    ...
    *,
    ...
    if json_output:
        _emit_json(event_payload(event), pretty=not streaming)
        return
    typer.echo(
        " ".join(
            (
...
def run_agent(
    ...
    if json_output:
        _emit_json({"run": run_payload(run), "session": session_payload(session)})
        return
    typer.echo(
        " ".join(
            (
...
def _decide_approval(
    *,
    ...
    if json_output:
        _emit_json(approval_payload(receipt))
        return
    typer.echo(
        " ".join(
            (
