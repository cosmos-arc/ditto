...
- **工具**:
    - **文件读写**：**必须使用**内置工具而非 Bash 命令
        ...
        - 创建目录 → 可以用 `mkdir`
    - **代码智能分析**：项目已配置 LSP。在以下场景**必须使用** LSP 工具而非 grep/ripgrep/glob，LSP 结果更精确，优先信任 LSP 的类型和诊断信息。
...
- **文档更新**：任务完成必须更新各级README.md及进度文档
- **其他**: 非本次修改引入的Lint检查问题也**必须**解决，避免日积月累！！
...
- 禁止在无合理原因下，绕过或忽略相关mypy、ruff、precommit检测
