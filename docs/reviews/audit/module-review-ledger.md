...
| BT-3 | backtest | 观察 | open | 测试覆盖文档（AGENTS.md）与实际文件不一致 | — |
...
| KERNEL-P2-02 | kernel | P2 | accepted | `Derived*` exceptions live in kernel for data/features sharing but are not part of the package type table in `packages/kernel/AGENTS.md`. | Kernel domain error ownership is underspecified. | Document them as a shared derived boundary or plan a later port-specific migration. |
