...
> 状态：原裁决保留；核心旅程、执行顺序和 ML/文本协议已由 2026-09-20 维护者裁决修订（见下文）。其他待审条款未默认通过。
...
本修订替代下文“先还全部 parity 债，再谈新交互”与第四节的全局串行顺序；交互效率票按上述核心旅程先交付，其余既有范围保留为后续批。图表已合入切片不重新实施。数据准入与估值收益已在各自决策票确认；ML/文本协议见下文当前修订。Agent 边界和架构简化仍待各自确认。
...
依据：[确定 ML 与文本研究协议：标签成熟、最终测试与前向验证](https://github.com/cosmos-arc/ditto/issues/244)。替代原模型管线与文本因子段落，不改变待确认的 D17。
