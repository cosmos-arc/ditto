...
| 文件路径 | 用途 |
|---------|------|
| `packages/core/src/ditto_core/engine/gates/__init__.py` | 门禁模块入口 |
| `packages/core/src/ditto_core/engine/gates/checkers.py` | 检查器实现 |
| `packages/core/src/ditto_core/engine/gates/report.py` | DQReport 模型 |

### 修改文件

| 文件路径 | 修改内容 |
|---------|---------|
| `packages/core/src/ditto_core/engine/specs.py` | 增加 `null_rate_limit` 字段 |
