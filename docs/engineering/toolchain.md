...
2026-09-06，macOS ARM64，Bun 1.3.14，独立缓存冷安装 151.51 秒，热安装 0.05 秒；Web 类型检查 25.11 秒。受网络、文件缓存与机器负载影响，这些数字仅作为同机反馈基线，不设任意性能门槛。Web 构建、208 文件/1757 项覆盖率测试、719 项原型测试、真实 API smoke 和 22 项优化器测试已通过。最终平台/整仓结果以本次实施记录及 CI 为准，不能由本页推断未运行的平台通过。
