...
> **适用范围**: Ditto 量化系统本地部署
>
> **注意**: 本文档已被 `docs/ops-manual.md`（v2.0）取代，保留仅供历史参考。
...
| 配置类 | 来源 | 配置文件 | 定义位置 |
|--------|------|----------|----------|
| `SystemSettings` | 字段名（大小写不敏感） | `system.env` | [settings.py:12-33](../packages/platform/src/ditto_platform/foundation/config/settings.py) |
| `ObservabilitySettings` | 字段名（大小写不敏感） | `observability.env` | [settings.py:36-76](../packages/platform/src/ditto_platform/foundation/config/settings.py) |
