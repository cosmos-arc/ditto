...
**日期：** 2026-08-12
**状态：** 进行中
**恢复点：** Wave 4 / Task 30，Campaign API、SSE、CLI 与崩溃恢复（Task 25 live 仍受 A3 阻塞）
...
```bash
pixi run -e dev pytest packages/application/tests/unit/process/experiments/test_autonomous_campaign.py packages/agent/tests/unit/tools/test_campaign_tool.py -v
pixi run -e dev check
```
...
```bash
pixi run -e dev pytest packages/application/tests/unit/process/experiments/test_generated_candidate_evaluator.py packages/analysis/tests/unit/experiments/test_generated_code.py -v
pixi run -e dev check
```
...
- **最新命令与结果：** Task 29 先观察 query/tool/command/approval/DI 缺失、非递归参数冻结、非法 evidence ref 被静默忽略、Agent→Analysis 间接依赖、application query cycle 与默认 DI 未拒绝等 RED；`c9d1372c` 最终落地 local/family/global PIT 查询、host-validated local 写入、独立证据与 exact HITL promotion、append-only revoke、content-addressed receipts、Agent-safe memory tool、真实 SQLite 跨 Campaign/revoke/PIT 集成和默认 disabled wiring。最终 55 个聚焦测试 passed，测试类型 0 errors，`pytest -m pit` 为 21 passed / 1 个缺失 TDX 样本的既有 skip，`arch-check` 为 43 kept / 0 broken 且 smells 通过；完整 `pixi run -e dev check` 为 lint/fmt/production type 通过、12,735 passed / 1 个既有 xfail、43 kept / 0 broken、architecture smells 通过、Harness validation 与 16 项 Harness tests 通过，`git diff --check` 通过。高风险 change review 复核 PIT cutoff、scope、审批重放、默认拒绝、存储前态/序号/known_at 触发器与公共合同后无剩余 finding。
