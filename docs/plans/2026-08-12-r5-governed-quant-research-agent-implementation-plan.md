...
**日期：** 2026-08-12
**状态：** 进行中
**恢复点：** Wave 4 / Task 28，候选新颖性与 multiple-testing ledger（Task 25 live 仍受 A3 阻塞）
...
- **下一步：** 继续 Task 28 的候选新颖性与 multiple-testing ledger；若 A3 的精确 runtime/profile、image digest、SBOM、dependency lock 与 seccomp approval 到位，再独立回填 Task 25 的物理 runner、固定构建 artifact 和 `sandbox_live` 攻击验收。获批前所有真实 OCI/gVisor acceptance 保持 fail closed。
...
- **最新命令与结果：** Task 27 先观察缺失 Campaign holdout bridge 的模块收集 RED，再观察 late approval/deep immutability 两项 RED 与跨 aggregate 签名错绑 RED；最终新桥接/隔离/既有 holdout 单元和真实 SQLite 集成 81 passed，测试类型 0 errors，目标 lint 与 `git diff --check` 通过；`pytest -m pit` 为 21 passed / 1 个缺失 TDX 样本的既有 skip，`arch-check` 为 43 kept / 0 broken 且 smells 通过。最终 `pixi run -e dev check` 为 lint/fmt/production type 通过、12,701 passed / 1 个既有 xfail、43 kept / 0 broken、architecture smells 通过、Harness validation 与 16 项 Harness tests 通过；PIT、审批/签名公共合同、错误处理、holdout redaction、一次性/并发/crash 恢复只读审查无阻断发现。
