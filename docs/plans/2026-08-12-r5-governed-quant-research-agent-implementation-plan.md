...
**日期：** 2026-08-12
**状态：** 进行中
**恢复点：** Wave 4 / Task 29，带 `known_at` 的研究记忆（Task 25 live 仍受 A3 阻塞）
...
- **下一步：** 继续 Task 29 的带 `known_at` 研究记忆；若 A3 的精确 runtime/profile、image digest、SBOM、dependency lock 与 seccomp approval 到位，再独立回填 Task 25 的物理 runner、固定构建 artifact 和 `sandbox_live` 攻击验收。获批前所有真实 OCI/gVisor acceptance 保持 fail closed。
...
- **最新命令与结果：** Task 28 先观察候选新颖性/SearchLedger bridge 模块收集 RED、Campaign 缺少新颖性证据 RED、未注册试验仍可评估 RED，以及回放替换不同有效新颖性凭证未被拒绝 RED；最终 31 个 novelty/ledger/Campaign 聚焦测试 passed，测试类型 0 errors，七个目标文件 lint/format 与 `git diff --check` 通过。实现完成后的 `pytest -m pit` 为 21 passed / 1 个缺失 TDX 样本的既有 skip，`arch-check` 为 43 kept / 0 broken 且 smells 通过；高风险复核前的完整 `pixi run -e dev check` 为 lint/fmt/production type 通过、12,717 passed / 1 个既有 xfail、43 kept / 0 broken、architecture smells 通过、Harness validation 与 16 项 Harness tests 通过。最终复核新增篡改、原始输出不落事件和凭证漂移测试，未发现剩余阻断项。
