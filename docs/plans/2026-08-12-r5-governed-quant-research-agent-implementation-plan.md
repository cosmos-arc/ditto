...
**日期：** 2026-08-12
**状态：** 进行中
**恢复点：** Wave 6 / Task 36，落地 retention、feature flags 与故障降级（Task 25 live 仍受 A3 阻塞）
...
- **下一步：** 进入 Wave 6 / Task 36，先以 29/30/31 天 retention 边界、长期工件误删、五个 feature flags 默认值和 provider/DB/sandbox/exporter 故障影响核心 Ditto 建立 RED；只实现 typed cleanup plan、dry-run 审计与显式执行边界，不执行任何真实清理。若 A3 的精确 runtime/profile、image digest、SBOM、dependency lock 与 seccomp approval 到位，再独立回填 Task 25 的物理 runner、固定构建 artifact 和 `sandbox_live` 攻击验收。
...
- **最新命令与结果：** Task 35 首轮聚焦测试因 `ditto_agent.observability` 与 Apps Agent OTel bridge 缺失在 collection 阶段 RED，并记录 R5.5 baseline evidence。最小实现后聚焦 13 项测试通过；首次架构门禁捕获 observability→runtime 与 runtime→observability 包内环，改为 observability-owned budget DTO 和 runtime 显式映射后为 43 kept / 0 broken。最终 `pixi run -e dev check` 为 lint/fmt/production type 通过、12,802 passed / 1 个既有 xfail、43 kept / 0 broken、architecture smells 通过、Harness validation 与 16 项 Harness tests 通过；测试类型 0 errors、`git diff --check` 通过。隐私/失败隔离 review 复核闭集字段、pre-export secret hash、无 raw state、低基数 metrics、默认 no-op、OTel 显式启用和 exporter exception containment 后无剩余 finding。
