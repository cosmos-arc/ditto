...
- [ ] Vite 8 + React 19 项目初始化（Bun 包管理）
- [ ] Tailwind CSS 4.x 配置（CSS-first，`@theme inline`）
- [ ] shadcn/ui v4 CLI 初始化
- [ ] Design Token CSS 变量实现
  - [ ] Primitive Token：6 色相 × 6 级色阶 + 12 级中性灰 + 8 色图表系列
  - [ ] Semantic Token：四色域（Market / Risk / System / Signal）× fg/bg/stroke
  - [ ] `:root`（light）+ `.dark`（dark-first）完整映射
  - [ ] Typography：Inter（UI）+ JetBrains Mono（技术字段）
  - [ ] Density 三级：comfortable(40px) / compact(32px) / ultra-compact(26px)
  - [ ] Motion 约束：token 级别开关（`--motion-flash-enabled`）
- [ ] shadcn/ui Token 桥接表映射（保留 shadcn 原生 token 名，值指向 Ditto Semantic）
- [ ] 渲染 Dashboard 空壳页面（侧边栏 + 顶栏 + 内容区骨架）
- [ ] 浏览器验证：色彩对比度、字体层级、间距节奏、暗色/亮色切换
