...
| 步骤 | 行动 | 验证 |
|------|------|------|
| 1 | 验证 Port 层单元测试并行安全性 | 运行 10+ 次，确保无 flaky |
| 2 | 逐步增加 Port 层并行度：`-n 2` → `-n 4` → `-n auto` | 每次验证稳定性 |
| 3 | 分析测试执行时间，识别慢测试 | pytest --durations=10 |
| 4 | 优化慢测试或标记为 `@pytest.mark.slow` | 确保快速测试可频繁运行 |
