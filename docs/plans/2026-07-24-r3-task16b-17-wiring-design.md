...
| 6b | `feat(api): expose governed research workflows`（剩余） | research experiment control routes（pause/cancel/resume/retry，需先补 control DI：`ExperimentControlProcess`/`Notifier` + 4 handler provider）+ experiment list route（需 analysis reader 扩展 `list_experiments`）+ CLI + research context + OpenAPI codegen 验证 | control DI 接入；experiment list reader；OpenAPI 零 diff |
