...
> **状态**：✅ Task 16b 已实施完成（2026-07-25，commit #3c 三子 commit `763f6d99`/`2646b067`/`9ca0dad5`）；✅ Task 17 commit #5（strategy governance 状态机 routes）已实施完成（2026-07-25）。commit #5 范围：application `commands/strategy_governance.py`（submit-review/approve/reject/deprecate/reactivate 5 handler，typed error 映射）+ `queries/strategy.py` 扩展 `list_versions`/`get_active` + contracts 加 `StrategyVersionInfo`/`StrategyActiveInfo`/`StrategyVersionStateInfo`/`StrategyActivePointerInfo` + governance `activate` 加 `expected_pointer_revision`（reactivate 乐观锁）+ governance `activate` 重构接收 `StrategyActivationEvent`（消除 PLR0913）+ apps governance DTO + 7 routes（list versions/get active/submit-review/approve/reject/deprecate/reactivate）+ DI。边界返回 application read model，不泄漏 capability 类型（apps route 不 import strategy.governance.models）。evidence-gated publish（经 `StrategyPromotionProcess`）待 ReviewPacket 链路就绪后独立暴露（commit #6 范围）。`pixi run -e dev check` 全绿（37 contracts + type/lint + test --fast）。✅ commit #6a（research catalog routes：GET /research/node-descriptors + /research/factors，application facade 包装静态 registry）已实施完成（2026-07-25）。剩余：commit #6b（experiment control routes 需补 control DI + experiment list 需 reader 扩展 + CLI + OpenAPI codegen 验证）+ evidence-gated publish（需 ReviewPacket 链路）+ W5（Task 18-22 ditto-app 前端）。
✅ commit #6b1（experiment list）+ commit #6b2（experiment control routes + CLI + research context + OpenAPI）已实施完成（2026-07-25，#6b2a `9e59a499` application DI：`_control_runtime.py` LoggingNotifier + ControlOnlyFirstAttemptFactory + retry_fold_under_transient_lease + RetryFoldControlRequest，coordinator.retry_fold 改 control 路径内联 transient lease 解决 LEASE_LOST 阻断，providers_process coordinator/notifier + providers_command 5 handler；#6b2b `c04e7c01` apps：models/research +3 DTO，research_experiment_routes +4 POST + _map_control_error，cli/commands/research 7 命令，registry/contexts/research + ResearchBundle，OpenAPI integration test 验证 4 control paths experimental maturity）。Task 17 后端 API 全部完成。剩余：evidence-gated publish（需 ReviewPacket 链路）+ W5（Task 18-22 ditto-app 前端）。
...
| 6a | ✅ `feat(api): expose research catalog`（2026-07-25） | research catalog routes（GET /research/node-descriptors + /research/factors）+ application `queries/research_catalog.py` `ResearchCatalogQueryFacade`（包装 `node_registry` + `R3_CORE_FACTOR_CATALOG`，返回 `NodeDescriptorInfo`/`FactorDescriptorInfo` application read model）+ apps DTO + DI provider | 静态 registry 暴露；apps route 不 import capability（facade 包装）；Task 18 strategy studio 前端依赖 |
| 6b1 | ✅ `feat(api): expose experiment list`（2026-07-25） | GET /research/experiments（list，newest first，无候选/fold 展开）+ analysis reader `list_experiments`（全量 SELECT experiment）+ `ExperimentReaderProtocol` 加方法 + `ExperimentQueryFacade.list_experiments` + `ExperimentSummaryReadModel` + apps DTO | reader 全量 list；facade summary 投影（O(experiments)，无 launch spec N+1）；route 复用 research_experiment_routes |
| 6b2 | ✅ `feat(api): expose experiment control`（2026-07-25） | #6b2a `9e59a499` application DI（`_control_runtime.py` LoggingNotifier + ControlOnlyFirstAttemptFactory + retry_fold_under_transient_lease + RetryFoldControlRequest，coordinator.retry_fold 内联 transient lease，providers coordinator/notifier/5 handler）+ #6b2b `c04e7c01` apps（+3 DTO + 4 POST routes + _map_control_error + CLI 7 命令 + research context + OpenAPI integration test） | control DI 接入（LoggingNotifier + placeholder factory）；retry_fold lease（内联 acquire/release）；OpenAPI paths experimental maturity |
