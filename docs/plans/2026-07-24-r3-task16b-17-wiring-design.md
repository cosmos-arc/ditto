...
| 6b2 | `feat(api): expose governed research workflows`（剩余） | experiment control routes（pause/cancel/resume/retry，**阻断**：需先补 control DI——`ExperimentControlProcess` 已在 coordinator.py，但 `ExperimentControlNotifier` 实现缺失 + Pause/Cancel/Resume/RetryFold 4 handler provider 未注册）+ CLI + research context + OpenAPI codegen 验证 | control DI 接入（Notifier 实现）；OpenAPI 零 diff |
